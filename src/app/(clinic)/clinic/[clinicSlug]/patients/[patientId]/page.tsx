...
export default function PatientProfilePage({
  ...
  const toolbarBtn = (icon: React.ReactNode, command: string, value?: string) => (
    <button
      ...
      className={`rounded p-1 transition-colors ${
        activeFormats[command] ? "bg-primary-light/30 text-primary-dark" : "text-gray-400 hover:text-gray-600"
      }`}
    >
      {icon}
    </button>
  ...
  if (error || !detail) {
    return <div className="py-20 text-center text-sm text-danger">بیمار یافت نشد.</div>;
  }

  const { patient, medicalAlerts } = detail;

  const alerts = [
    medicalAlerts.hasAllergy && { text: medicalAlerts.allergyDescription || "سابقه‌ی حساسیت", tone: "bg-danger" },
    medicalAlerts.hasSpecialDisease && {
      text: medicalAlerts.specialDiseaseDescription || "بیماری خاص ثبت‌شده",
      tone: "bg-warning",
    },
    medicalAlerts.usesMedicine && { text: medicalAlerts.medicineDescription || "مصرف دارو", tone: "bg-warning" },
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-5">
        <div className="flex flex-wrap items-center gap-8">
          <div className="flex items-center gap-3">
            <Image src="/image/user.PNG" alt="User" width={50} height={50} unoptimized className="rounded-full object-cover" />
            <div>
          ...
          <InfoStat
            label="تاریخ تولد / سن"
            value={`${formatJalaliDate(patient.birthDate)}${patient.age != null ? ` - ${patient.age.toLocaleString("fa-IR")} سال` : ""}`}
          />
          ...
          <InfoStat
            ...
            valueTone={debt && debt > 0 ? "text-danger" : undefined}
          />
        </div>
      </div>
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 whitespace-nowrap border-b-2 py-3 transition-colors ${
                  activeTab === tab.key ? "border-primary font-medium text-primary-dark" : "border-transparent text-gray-500 hover:text-primary"
                }`}
              >
            ...
            <div className="space-y-4">
              <div className="rounded-2xl border border-gray-100 bg-white p-4">
                ...
                <div className="mt-2 flex items-center justify-between border-t border-gray-50 pt-2">
                  <span className="text-[10px] text-gray-300">ثبت توسط: دکتر سارا محمدی - ۱۴۰۳/۰۳/۲۱ - ۱۱:۳۰</span>
                  ...
                  <tbody>
                    {RECENT_SERVICES.map((s) => (
                      <tr key={s.name} className="border-b border-gray-50">
                        ...
                        <td className="py-2">
                          <span className="rounded-full bg-primary-light/20 px-2 py-0.5 text-primary-dark">{s.status}</span>
                        </td>
            ...
            <div className="space-y-4">
              <div className="rounded-2xl border border-gray-100 bg-white p-4">
                <div className="mb-3 flex items-center">
                  ...
                  <h3 className="mr-2 text-xs font-bold text-gray-800">نوبت بعدی</h3>
                </div>
                {nextAppointment ? (
                  <>
                    <div className="flex items-center gap-1.5 text-xs text-gray-600">
                      <CalendarDays className="h-3.5 w-3.5 text-gray-300" />
                      {formatJalaliDate(nextAppointment.startTime)} ساعت {formatJalaliTime(nextAppointment.startTime)}
                    </div>
                    <div className="mt-1 text-xs text-gray-400">{nextAppointment.serviceName}</div>
                  </>
                ) : (
                  <div className="text-xs text-gray-400">نوبت آینده‌ای ثبت نشده.</div>
                )}
                <button className="mt-3 w-full rounded-lg bg-primary-light/15 py-2 text-[11px] font-medium text-primary-dark">
                  رزرو / ویرایش نوبت
                </button>
              ...
              <div className="rounded-2xl border border-gray-100 bg-white p-4">
                ...
                <div className="space-y-2">
                  {alerts.length > 0 ? (
                    ...
                    <div className="text-xs text-gray-400">هشداری ثبت نشده.</div>
                  )}
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-gray-100 bg-white p-4">
                ...
                />
                <button className="mt-2 w-full rounded-lg border border-gray-200 py-2 text-[11px] text-gray-600">ثبت یادداشت</button>
              </div>
            </div>
          </div>
...
function InfoStat({
  ...
  return (
    <div className={`rounded-xl border border-gray-200 bg-white px-4 py-3 transition-all duration-200 hover:border-primary/40 hover:shadow-md ${className}`}>
      <div className="text-[11px] font-medium text-gray-400">{label}</div>
      {custom ?? <div className={`mt-2 text-sm ${valueTone ?? "text-gray-800"}`}>{value}</div>}
    </div>
  );
}